...
def load_data():
    f = open('./zscore40_1D.pkl', 'rb')
    mean1, std1= cPickle.load(f)
    return mean1, std1
...
def read_IEMOCAP():
    ...
    rootdir = '/home/xiaoxiao.zhen/Zwave'

    mean1, std1 = load_data()
    ...
    train_data = np.empty((train_num, 300, filter_num), dtype=np.float32)
    test_data = np.empty((test_num, 300, filter_num), dtype=np.float32)
    valid_data = np.empty((valid_num, 300, filter_num), dtype=np.float32)
                            ...
                            if (speaker in ['Session1', 'Session2', 'Session3', 'Session4']):
                                ...
                                if (time <= 300):
                                    ...
                                    #delta21 = np.pad(delta21, ((0, 300 - delta21.shape[0]), (0, 0)), 'constant',
                                     #                constant_values=0)
                                    train_data[train_num, :, :] = (part - mean1) / (std1 + eps)
                                ...
                                if (wavname[-4] == 'M'):
                                    ...
                                    if (time <= 300):
                                        ...
                                        #delta21 = np.pad(delta21, ((0, 300 - delta21.shape[0]), (0, 0)), 'constant',
                                        #                 constant_values=0)
                                        test_data[test_num, :, :] = (part - mean1) / (std1 + eps)
                                    ...
                                    if (time <= 300):
                                        ...
                                        #delta21 = np.pad(delta21, ((0, 300 - delta21.shape[0]), (0, 0)), 'constant',
                                                         #constant_values=0)
                                        valid_data[valid_num, :, :] = (part - mean1) / (std1 + eps)
    ...
    for m in range(1):
        ...
        hap_data = np.empty((pernum, 300, filter_num), dtype=np.float32)
        neu_data = np.empty((pernum, 300, filter_num), dtype=np.float32)
        sad_data = np.empty((pernum, 300, filter_num), dtype=np.float32)
        ang_data = np.empty((pernum, 300, filter_num), dtype=np.float32)
