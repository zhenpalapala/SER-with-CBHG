...
from acbhg_11 import acrnn
#from acbhg_01 import acrnn
...
tf.app.flags.DEFINE_string  ('logdir', './logdir','logdir for tensorboard')
...
def train():
    ...
    ##########tarin model###########
    X = tf.placeholder(tf.float32, shape=[None, FLAGS.image_height,FLAGS.image_width])
    Y = tf.placeholder(tf.int32, shape=[None, FLAGS.num_classes])
    #input_length=tf.placeholder(tf.int32, [None], 'input_lengths')

    is_training = tf.placeholder(tf.bool)
    lr = tf.placeholder(tf.float32)
   # keep_prob = tf.placeholder(tf.float32)
    Ylogits = acrnn(X, is_training=is_training, dropout_keep_prob=FLAGS.dropout_keep_prob)
    ...
    with tf.Session() as sess:
        ...
        for i in range(FLAGS.num_epoch):
            ...
            [_,tcost,tracc] = sess.run([train_op,cost,accuracy], feed_dict={X:train_data[start:end,:,:], Y:train_label[start:end,:],is_training:True,  lr:FLAGS.learning_rate})
            if i % 5 == 0:
                #for valid data
                ...
                for v in range(valid_iter):
                    ...
                    if(v == valid_iter-1):
                        if(v_end < valid_size):
                            v_end = valid_size
                    loss, y_pred_valid[v_begin:v_end,:] = sess.run([cross_entropy,Ylogits],feed_dict = {X:valid_data[v_begin:v_end],Y:Valid_label[v_begin:v_end],is_training:False})
                    cost_valid = cost_valid + np.sum(loss)
            ...
            summary_str=sess.run(merged_summary_op, feed_dict = {X:train_data[start:end,:,:], Y:train_label[start:end,:],
                                            is_training:True, lr:FLAGS.learning_rate})
