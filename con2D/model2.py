...
tf.app.flags.DEFINE_string  ('save_dir', './checkpoint/', 'the checkpoint dir')
tf.app.flags.DEFINE_string  ('model_name', 'model3.ckpt', 'model name')
tf.app.flags.DEFINE_string  ('logdir', './logdir','logdir for tensorboard')
tf.app.flags.DEFINE_boolean  ('restore', True,'whether to restore ckpt model')
...
def train():
    ...
    checkpoint_path=os.path.join(FLAGS.save_dir,FLAGS.model_name)
        ...
        if FLAGS.restore:
            ...
            try:
                ...
                if (checkpoint_state and checkpoint_state.model_checkpoint_path):
                    ...
                    log('No model to load at {}'.format(FLAGS.save_dir), slack=True)
